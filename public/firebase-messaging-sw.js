...
const logToConsole = (level, message, data = null) => {
  ...
  const logMessage = `[${timestamp}] [firebase-messaging-sw.js] ${message}`;
  ...
  if (level === 'error') {
    try {
      const errors = JSON.parse(self.localStorage?.getItem('sw_notification_errors') || '[]');
      errors.unshift({
        timestamp,
        message,
        data,
      ...
      self.localStorage?.setItem('sw_notification_errors', JSON.stringify(errors));
    } catch (e) {
...
messaging.onBackgroundMessage((payload) => {
  logToConsole('info', 'Received background message', {
  ...
  self.registration.showNotification(notificationTitle, notificationOptions)
    .then(() => {
      logToConsole('info', 'Notification shown successfully on lock screen/background', {
    ...
    .then(() => {
      // Send delivery confirmation to main thread
      self.clients.matchAll().then(clients => {
        clients.forEach(client => {
          client.postMessage({
            type: 'NOTIFICATION_DELIVERY_STATUS',
            notificationId: notificationOptions.tag || `bg_${Date.now()}`,
            status: 'delivered',
            timestamp: Date.now()
          });
        });
      ...
      logToConsole('info', 'Background notification delivered successfully', {
        title: notificationTitle,
        tag: notificationOptions.tag,
        platform: isIOS() ? 'iOS' : isAndroid() ? 'Android' : 'Desktop'
      });
    ...
    .catch(error => {
      // Send failure confirmation to main thread
      self.clients.matchAll().then(clients => {
        clients.forEach(client => {
          client.postMessage({
            type: 'NOTIFICATION_DELIVERY_STATUS',
      ...
      logToConsole('error', 'Error showing notification', {
        error: error.message,
        title: notificationTitle,
        platform: isIOS() ? 'iOS' : isAndroid() ? 'Android' : 'Desktop',
...
self.addEventListener('notificationclick', (event) => {
  logToConsole('info', 'Notification click received', {
  ...
  // Send click confirmation to main thread
  self.clients.matchAll().then(clients => {
    clients.forEach(client => {
      client.postMessage({
        type: 'NOTIFICATION_DELIVERY_STATUS',
        notificationId: event.notification.tag || `click_${Date.now()}`,
        status: 'clicked',
        timestamp: Date.now()
      });
    });
  ...
  logToConsole('info', 'Will navigate to URL', {
    url: urlToOpen,
    notificationData,
    platform: isIOS() ? 'iOS' : isAndroid() ? 'Android' : 'Desktop'
  });
  ...
  event.waitUntil(
    clients.matchAll({ type: 'window', includeUncontrolled: true }).then((windowClients) => {
      ...
      if (matchingClient) {
        logToConsole('info', 'Found existing client, focusing and navigating', {
          clientUrl: matchingClient.url,
          targetUrl: urlToOpen
        });
        return matchingClient.focus().then(() => {
          ...
          return new Promise(resolve => {
            setTimeout(() => {
              matchingClient.navigate(urlToOpen).then(resolve).catch(navError => {
                logToConsole('error', 'Navigation failed', {
      ...
      if (anyClient) {
        logToConsole('info', 'Using any available client', { clientUrl: anyClient.url });
        return anyClient.focus().then(() => {
          return new Promise(resolve => {
            setTimeout(() => {
              anyClient.navigate(urlToOpen).then(resolve).catch(navError => {
                logToConsole('error', 'Navigation failed on fallback client', {
                  error: navError.message,
                  targetUrl: urlToOpen
                });
                resolve();
      ...
      logToConsole('info', 'No existing clients found, opening new window', { url: urlToOpen });
      return clients.openWindow(urlToOpen).catch(openError => {
        logToConsole('error', 'Failed to open new window', {
...
self.addEventListener('push', (event) => {
  logToConsole('info', 'Push event received', {
    hasData: !!event.data,
    platform: isIOS() ? 'iOS' : isAndroid() ? 'Android' : 'Desktop'
  });
  ...
  try {
    ...
    event.waitUntil(
      self.registration.showNotification(title, options)
        .then(() => {
          ...
          self.clients.matchAll().then(clients => {
            clients.forEach(client => {
              client.postMessage({
                type: 'NOTIFICATION_DELIVERY_STATUS',
                notificationId: options.tag || `push_${Date.now()}`,
                status: 'delivered',
                timestamp: Date.now()
              });
            });
          });
          logToConsole('info', 'Push notification shown successfully', {
            title,
            platform: isIOS() ? 'iOS' : isAndroid() ? 'Android' : 'Desktop'
          });
        })
        .catch(error => {
          ...
          logToConsole('error', 'Failed to show push notification', {
            error: error.message,
            title,
            platform: isIOS() ? 'iOS' : isAndroid() ? 'Android' : 'Desktop'
          });
    ...
    logToConsole('warn', 'Push event was not JSON, using text fallback', { error: e.message });
