...
// Check if iOS PWA supports native Web Push API
export const isIOSPwaWithNativeWebPush = (): boolean => {
  if (!isIOSPwa()) return false;
  
  // Check if running as PWA and has iOS 16.4+ native web push support
  return isIOSWebPushSupported() && 'PushManager' in window && 'serviceWorker' in navigator;
};
...
// Check iOS version for web push compatibility
export const getIOSVersion = (): { major: number; minor: number } | null => {
...
// Note: Even iOS 16.4+ PWAs have limitations with Firebase messaging
export const isIOS164PlusWithWebPush = (): boolean => {
  ...
  // Check iOS version
  if (!isIOSWebPushSupported()) return false;
  
  // Check if PushManager is available (but note: FCM still won't work)
  return 'PushManager' in window;
};
...
// iOS PWA native Web Push registration
export const registerIOSPwaWebPush = async (): Promise<{
  success: boolean;
  status: NotificationPermission | 'unsupported' | 'error';
  error?: string;
}> => {
  try {
    if (!isIOSPwaWithNativeWebPush()) {
      return {
        success: false,
        status: 'unsupported',
        error: 'iOS PWA native web push not supported on this device/version'
      };
    }
    
    console.log('iOS PWA with native web push detected, requesting permissions');
    ...
    // Register service worker for iOS PWA push
    if ('serviceWorker' in navigator) {
      try {
        const registration = await navigator.serviceWorker.register('/ios-pwa-push-sw.js');
        console.log('iOS PWA push service worker registered:', registration);
        
        // Subscribe to push notifications using native Web Push API
        if ('PushManager' in window && registration.pushManager) {
          const subscription = await registration.pushManager.subscribe({
            userVisibleOnly: true,
            applicationServerKey: urlBase64ToUint8Array('BLW7VJrM3F8oL2IFysoC7monAgQ_dTWeaZZU3y3Hp0SgGK0C_jPBqknMcMs4v6v6NxJAaa0mqJDoNEn3Ce1Y0F8')
          });
          ...
          return {
            success: false,
            status: 'error',
            error: 'PushManager not available in service worker'
          };
        }
      } catch (swError) {
        console.error('Error registering iOS PWA service worker:', swError);
        return {
          success: false,
          status: 'error',
          error: 'Failed to register iOS PWA service worker'
        };
      }
    } else {
      return {
    ...
    console.error('Error in iOS PWA web push registration:', error);
    return {
      success: false,
      status: 'error',
...
const requestSafariPermission = async (): Promise<{
  ...
  try {
    ...
    } else if (isIOSPwaWithNativeWebPush()) {
      console.log('iOS PWA with native web push detected, using iOS-specific approach');
      return await registerIOSPwaWebPush();
...
// Cross-platform notification creation with special handling for Safari and Android
export const createNotification = async (title: string, options: NotificationOptions & { 
  data?: any;
  timestamp?: number;
}): Promise<boolean> => {
  try {
    ...
    // For Android Chrome and when service worker is available, use ServiceWorkerRegistration.showNotification
    if (isAndroid() && isServiceWorkerSupported()) {
      try {
        const registration = await navigator.serviceWorker.ready;
        if (registration) {
          console.log('Using ServiceWorkerRegistration.showNotification for Android');
    ...
    // For all other cases or as fallback, use direct Notification constructor
