...
import { notificationDeliveryTracker, type DeliveryStatus } from './notificationDeliveryTracker';
import { 
  isAndroid, 
  isChrome, 
  isIOS,
...
// Initialize Firebase Messaging (with iOS Safari compatibility checks)
let messaging: any = null;
let messagingInitError: string | null = null;

const canInitializeMessaging = () => {
  // Don't initialize Firebase messaging on iOS - iOS PWA uses native Web Push API
  if (isIOS()) {
    console.log('iOS detected: Using native Web Push API instead of Firebase messaging');
    return false;
  }
...
try {
  if (canInitializeMessaging()) {
    ...
    console.log('Skipping Firebase messaging initialization - not supported on this platform');
    messagingInitError = 'Platform not supported for Firebase messaging';
  }
} catch (error) {
  console.error('Error initializing Firebase messaging:', error);
  messagingInitError = (error as Error).message;
}
...
const isFcmSupported = (): boolean => {
  // If messaging initialization failed, FCM is not supported
  if (messaging === null || messagingInitError !== null) {
    return false;
  }
  
  // iOS PWA uses native Web Push API, not Firebase messaging
  if (isIOS()) {
    console.log('iOS detected: Firebase messaging not supported, using native Web Push API');
    return false;
  }
  
  // For Safari on macOS, FCM may work but with limitations
  if (isSafari() && isMacOS()) {
    return isWebPushSupported() && isServiceWorkerSupported();
  }
  
  // For all other browsers, check for service worker and messaging
  return isServiceWorkerSupported() && isWebPushSupported();
};
...
const requestFcmToken = async (): Promise<string | null> => {
  if (!isFcmSupported()) {
    ...
    // iOS PWA uses native Web Push API instead of Firebase messaging
    if (isIOS()) {
      console.log('iOS detected - Using native Web Push API for notifications');
      
      // Check if iOS PWA subscription exists
      const iosSubscription = localStorage.getItem('iosPwaSubscription');
      if (iosSubscription) {
        console.log('iOS PWA subscription found in localStorage');
        return 'ios-pwa-subscription';
      }
...
// Show a notification using our cross-platform utility with delivery tracking
const showNotification = async (title: string, options?: any): Promise<boolean> => {
  ...
  if (lastNotificationId === notificationHash && (now - lastNotificationTimestamp) < 2000) {
    console.log('Preventing duplicate notification:', notificationHash);
    return false;
  }
  
  // Update last notification tracking
  lastNotificationId = notificationHash;
  lastNotificationTimestamp = now;
  
  // Track notification delivery
  const platform = isIOS() ? 'iOS' : isAndroid() ? 'Android' : 'Desktop';
  notificationDeliveryTracker.trackNotification(notificationId, title, platform, (status) => {
    console.log(`Notification ${notificationId} status:`, status);
  });
  
  // Ensure options has the notification ID
  ...
  try {
    return await createNotification(title, options);
  } catch (error) {
...
const showTimerNotification = async (timerName: string): Promise<boolean> => {
  return await showNotification(`${timerName} has ended!`, {
...
const showReplyNotification = async (
  fromName: string, 
  replyMessage: string, 
  alertId: string,
  userType: 'admin' | 'chair' | 'press' | 'logistics' = 'admin'
): Promise<boolean> => {
  ...
  const targetRoute = userType === 'admin' ? '/admin-panel' : 
                      ...
                      userType === 'logistics' ? '/logistics-dashboard' :
                      '/chair-dashboard';
...
const getDeliveryStats = () => {
  return notificationDeliveryTracker.getStats();
};

// Get recent deliveries
const getRecentDeliveries = () => {
  return notificationDeliveryTracker.getAllDeliveries().slice(0, 10);
};
