...
import { PressMembersList } from '@/components/press/PressMembersList';
...
const PressDashboard = () => {
    ...
    if (alertsData && Array.isArray(alertsData)) {
      const userAlerts = alertsData
        ...
        .map(alert => ({
          ...
          reply: alert.reply,
          admin: alert.admin,
          from: alert.chairName || alert.admin
        }))
  ...
  }, [alertsData, user?.council, user?.id]);
  ...
  return (
    <PressLayout activeItem="/press-dashboard">
      <div className="space-y-6">
        ...
        <div className="animate-fade-in">
          ...
          {/* Press Messaging Section */}
          <div className="space-y-4 mb-8">
            <h2 className="text-lg font-medium text-primary dark:text-white">Press Team Communication</h2>
            <PressMembersList users={users} user={user} />
          </div>
                  ...
                  {recentAlerts.slice(0, 3).map((alert) => (
                    <div key={alert.id} className="py-3 first:pt-0 last:pb-0 flex items-start gap-3">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        {(alert.admin || alert.from) && alert.reply && (
