...
// Member Mobile Navigation Component
const MemberMobileNav: React.FC = () => {
  const [isOpen, setIsOpen] = useState(false);
  const location = useLocation();
  const { user, logout } = useAuth();
  
  // Only show for member users
  const isMember = user && (user.role === 'member-hcc' || user.role === 'member-fcc');
  ...
  const councilColor = user?.role === 'member-hcc' ? 'text-blue-600' : 'text-green-600';
            ...
            <AnimatePresence>
              {isOpen && (
                <motion.div
                  ...
                  className="mb-3 pl-3"
                >
                  <div className="text-xs font-medium text-black/70">{councilName} Member</div>
                  <div className={`text-sm font-bold ${councilColor}`}>{user?.username}</div>
                </motion.div>
              )}
            </AnimatePresence>
