...
import { Menu, X, LayoutDashboard, Settings, LogOut, FileText } from "lucide-react";
...
const GlassEffect: React.FC<GlassEffectProps> = ({
  ...
  return (
    <div
      className={`relative flex font-semibold overflow-hidden text-black cursor-pointer transition-all duration-700 rounded-3xl ${className}`}
      style={glassStyle}
      onClick={onClick}
    >
      {/* Glass Layers */}
...
// R&T Admin Mobile Navigation Component
const RTAdminMobileNav: React.FC = () => {
  const [isOpen, setIsOpen] = useState(false);
  const location = useLocation();
  const { user, logout } = useAuth();
  
  // Only show for admin-rt users
  const isRTAdmin = user && user.role === 'admin-rt';
  
  if (!isRTAdmin) return null;
            ...
            <AnimatePresence>
              {isOpen && (
                <motion.div
                  ...
                  className="mb-3 pl-3"
                >
                  <div className="text-xs font-medium text-black/70">R&T Admin</div>
                  <div className="text-sm font-bold text-purple-600">{user?.username}</div>
                </motion.div>
              )}
            </AnimatePresence>
...
export default RTAdminMobileNav;