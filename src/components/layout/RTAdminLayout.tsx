...
import { LayoutDashboard, Settings as SettingsIcon, LogOut, ChevronLeft, FileText } from 'lucide-react';
import NotificationInitializer from '../NotificationInitializer';
import RTAdminMobileNav from './RTAdminMobileNav';

interface RTAdminLayoutProps {
  activeItem?: string;
  children: React.ReactNode;
}

const RTAdminLayout: React.FC<RTAdminLayoutProps> = ({ activeItem, children }) => {
      ...
      >
        <div className="h-full flex flex-col py-5">
          ...
          <div className={cn("px-6 mb-6", isCollapsed && !isHovered && "px-2")}>
            ...
            {shouldShowContent && (
              <>
                <h2 className="text-xl font-bold text-primary dark:text-white text-center mt-3">R&T Admin</h2>
