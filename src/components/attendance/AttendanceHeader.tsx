...
export const AttendanceHeader: React.FC<AttendanceHeaderProps> = ({
  ...
  return (
    <motion.div 
      className={`flex flex-col ${isMobile ? 'gap-3 mb-4' : 'md:flex-row md:items-center justify-between gap-4 mb-6'}`}
    ...
    >
      ...
      >
        <h1 className={`${isMobile ? 'text-xl' : 'text-2xl md:text-3xl'} font-bold bg-clip-text text-transparent bg-gradient-to-r from-primary to-primary/70`}>
          Attendance Management
        </h1>
        <motion.div 
          className={`flex ${isMobile ? 'flex-col gap-1' : 'items-center'} text-gray-500 mt-1`}
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
        ...
        >
          ...
          {!isMobile && <ChevronRight size={14} className="mx-1 opacity-70" />}
          <span className={`${isMobile ? 'text-xs ml-5' : 'text-sm'} italic opacity-80`}>
      ...
      <motion.div 
        className={`flex ${isMobile ? 'flex-col gap-2' : 'flex-col sm:flex-row items-center gap-3'}`}
        initial={{ opacity: 0, y: 10 }}
        animate={{ opacity: 1, y: 0 }}
        transition={{ delay: 0.4, duration: 0.5 }}
      >
        {isMobile ? (
          // Mobile layout: Date selector and refresh button in same row
          <div className="flex items-center gap-2">
            <div className="flex items-center gap-1 p-1 bg-white/80 backdrop-blur-sm rounded-lg shadow-sm border border-gray-100 flex-1">
              <motion.div
                whileHover={{ y: -2 }}
                whileTap={{ y: 1 }}
              >
            ...
            >
              <Button 
                ...
                size="sm"
                className="bg-white shadow-sm hover:shadow-md transition-all text-xs px-3"
              >
                {isRefreshing ? (
